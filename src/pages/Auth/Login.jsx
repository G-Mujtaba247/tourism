        ...
        <div className="w-full max-w-sm mt-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex text-xs mt-2">
               <Link to="/forgot-password" className="text-xs text-gray-800 hover:text-green-800">
                  Forgot Password? (کیا آپ پاس ورڈ بھول گئے ہیں؟)
               </Link>
            </div>

            <button
              type="submit"
              className="w-full bg-[#009b4d] hover:bg-[#008040] text-white font-semibold py-3 rounded-md transition duration-200 mt-4 shadow-sm"
              disabled={isLoading}
            >
              {isLoading ? "Signing in..." : "Login (لاگ ان)"}
            </button>
          </form>

          <div className="mt-8 border-dashed border-2 border-green-500 rounded-lg p-4 text-center bg-transparent">
            <p className="text-[13px] font-bold text-gray-800">
              Don't have an account? <Link to="/signup" className="text-green-600 hover:underline ml-1">REGISTER NOW</Link>
            </p>
            <p className="text-xs text-green-700 mt-1 font-semibold">(کیا آپ کا اکاؤنٹ نہیں ہے؟ ابھی رجسٹر کریں)</p>
          </div>
